/**
 * Try to reverse decode from received symbols
 */
#include <iostream>
#include <fstream>
#include <vector>
...
int main() {
    ...
    cout << "=== Decoded message (whatever it is) ===" << endl;
    string output;
    ...
    // Show hex dump
    cout << "\n=== Hex dump ===" << endl;
    for (size_t i = 0; i + 8 <= decoded.size() && i < 200*8; i += 8) {
        uint8_t byte = 0;
        for (int j = 0; j < 8; j++) byte = (byte << 1) | (decoded[i + j] & 1);
        printf("%02x ", byte);
        if ((i/8 + 1) % 16 == 0) cout << endl;
    }
    cout << endl;
    
    // Also try without Viterbi (just map tribits directly to bits)
    cout << "\n=== Direct tribit decode (no Viterbi) ===" << endl;
    scr.reset();
    idx = 0;
    vector<int> tribits;
    while (tribits.size() < 100 && idx < (int)result.data_symbols.size()) {
        for (int i = 0; i < 32 && tribits.size() < 100 && idx < (int)result.data_symbols.size(); i++) {
            int pos = decode_8psk_position(result.data_symbols[idx++]);
            int scr_val = scr.next();
            int gray = (pos - scr_val + 8) % 8;
            int tribit = inv_mgd3[gray];
    ...
    cout << "First 100 tribits: ";
    for (int t : tribits) cout << t;
    cout << endl;
