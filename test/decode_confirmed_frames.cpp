/**
 * Decode with confirmed frame boundaries
 */
#include <iostream>
#include <fstream>
#include <vector>
...
int main() {
    ...
    const int BLOCK_SYMBOLS = BLOCK_BITS / 3;  // 960 data symbols needed
    ...
    cout << "Need " << BLOCK_SYMBOLS << " data symbols" << endl;
    
    while (data_count < BLOCK_SYMBOLS && idx < (int)result.data_symbols.size()) {
        // 32 data symbols
        for (int i = 0; i < UNKNOWN_LEN && data_count < BLOCK_SYMBOLS && idx < (int)result.data_symbols.size(); i++) {
        ...
        // 16 probe symbols - skip and advance scrambler
    ...
    cout << "Processed " << data_count << " data symbols from " << idx << " total" << endl;
    cout << "Scrambler advanced " << scr_count << " times" << endl;
    ...
    // Debug: show first few tribits
    cout << "\nFirst 20 descrambled tribits: ";
    RefDataScrambler scr2;
    for (int i = 0; i < 20; i++) {
        int pos = decode_8psk_position(result.data_symbols[i]);
