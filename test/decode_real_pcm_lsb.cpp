/**
 * Decode real PCM with LSB-first - trace the differences
 */
#include <iostream>
#include <fstream>
#include <vector>
...
int main() {
    ...
    // Generate EXPECTED TX symbols (with LSB-first)
    vector<int> msg_bits;
    for (const char* p = TEST_MSG; *p; p++) {
        uint8_t c = *p;
        for (int i = 0; i < 8; i++) msg_bits.push_back((c >> i) & 1);  // LSB first
    }
    ...
    int tx_count = 0;
    while (tx_count < BLOCK_BITS / 3) {
        for (int i = 0; i < 32 && tx_count < BLOCK_BITS / 3; i++) {
            int tribit = (interleaver.fetch() << 2) | (interleaver.fetch() << 1) | interleaver.fetch();
            int gray = mgd3[tribit];
            expected_symbols.push_back((gray + tx_scr.next()) % 8);
            tx_count++;
        }
        for (int i = 0; i < 16; i++) expected_symbols.push_back(tx_scr.next());
    }
    ...
    vector<int> received_symbols;
    for (size_t i = 0; i < result.data_symbols.size(); i++) {
        received_symbols.push_back(decode_8psk_position(result.data_symbols[i]));
    }
    
    // Compare and show differences
    cout << "=== Symbol Comparison ===" << endl;
    int sym_match = 0;
    vector<int> diff_positions;
    for (size_t i = 0; i < min(expected_symbols.size(), received_symbols.size()); i++) {
        if (expected_symbols[i] == received_symbols[i]) {
            sym_match++;
        } else {
            diff_positions.push_back(i);
        }
    }
    cout << "Symbol matches: " << sym_match << "/" << min(expected_symbols.size(), received_symbols.size()) << endl;
    cout << "Differences at positions: ";
    for (int pos : diff_positions) {
        if (pos < 200) cout << pos << "(exp=" << expected_symbols[pos] << ",rcv=" << received_symbols[pos] << ") ";
    }
    cout << endl;
    ...
    while (rx_data_count < BLOCK_BITS / 3 && rx_idx < (int)received_symbols.size()) {
        for (int i = 0; i < 32 && rx_data_count < BLOCK_BITS / 3 && rx_idx < (int)received_symbols.size(); i++) {
            int pos = received_symbols[rx_idx++];
        ...
        for (int i = 0; i < 16 && rx_idx < (int)received_symbols.size(); i++) {
            rx_idx++;
            rx_scr.next();
        }
    }
    ...
    // Compare soft bits to expected encoded bits
    int bit_match = 0;
    for (int i = 0; i < BLOCK_BITS; i++) {
        int rx_bit = soft[i] > 0 ? 0 : 1;
        if (rx_bit == encoded[i]) bit_match++;
    }
    cout << "\nEncoded bit matches: " << bit_match << "/" << BLOCK_BITS << endl;
