/**
 * Search for correct start position by trying all positions
 */
#include <iostream>
#include <fstream>
#include <vector>
...
int try_decode(const vector<complex<float>>& symbols, int start, int scr_start) {
    ...
    // Advance scrambler to scr_start
    for (int i = 0; i < scr_start; i++) scr.next();
    ...
    while (data_count < BLOCK_SYMBOLS && idx < (int)symbols.size()) {
        for (int i = 0; i < 32 && data_count < BLOCK_SYMBOLS && idx < (int)symbols.size(); i++) {
    ...
    if (data_count < BLOCK_SYMBOLS) return 0;
...
int main() {
    ...
    cout << "\nSearching all start positions and scrambler offsets..." << endl;
    ...
    for (int start = 0; start < 50; start++) {
        for (int scr = 0; scr < 160; scr++) {
            int matches = try_decode(result.data_symbols, start, scr);
            if (matches > best_matches) {
                best_matches = matches;
                best_start = start;
                best_scr = scr;
                if (matches > 5) {
                    cout << "Found: start=" << start << " scr=" << scr << " matches=" << matches << endl;
                }
            }
        }
    }
    
    cout << "\nBest: start=" << best_start << " scr=" << best_scr << " matches=" << best_matches << "/" << EXPECTED_LEN << endl;
