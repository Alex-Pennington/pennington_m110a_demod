/**
 * Try continuing scrambler from preamble position
 */
#include <iostream>
#include <fstream>
#include <vector>
...
int try_decode(const vector<complex<float>>& data_symbols, int scrambler_advance) {
    // Advance scrambler by preamble length
    RefScrambler scr;
    for (int i = 0; i < scrambler_advance; i++) {
        scr.next_tribit();
    }
    ...
    while (sym_idx + unknown_len + known_len <= (int)data_symbols.size()) {
        for (int i = 0; i < unknown_len; i++) {
            complex<float> sym = data_symbols[sym_idx + i];
...
int main() {
    ...
    // Short preamble = 3 frames × 480 = 1440 symbols
    cout << "\n--- Trying different scrambler advances ---" << endl;
    for (int advance = 0; advance <= 1500; advance += 100) {
        int matches = try_decode(result.data_symbols, advance);
        if (matches > 1) {
            cout << "Advance " << advance << ": " << matches << "/" << EXPECTED_LEN << " matches" << endl;
        }
    }
    
    // Try around 1440
    cout << "\n--- Fine search around 1440 ---" << endl;
    for (int advance = 1430; advance <= 1450; advance++) {
        int matches = try_decode(result.data_symbols, advance);
        cout << "Advance " << advance << ": " << matches << "/" << EXPECTED_LEN << " matches" << endl;
    }
