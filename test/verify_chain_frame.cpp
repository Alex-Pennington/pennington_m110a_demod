/**
 * Verify the encode/decode chain with frame structure - debug version
 */
#include <iostream>
#include <vector>
...
int main() {
    ...
    while (tx_data_count < BLOCK_SYMBOLS) {
        // 32 data symbols
        for (int i = 0; i < UNKNOWN_LEN && tx_data_count < BLOCK_SYMBOLS; i++) {
        ...
        // 16 probe symbols (data = 0)
        for (int i = 0; i < KNOWN_LEN; i++) {
            int scr_val = tx_scr.next();
            tx_symbols.push_back(scr_val);  // Probe = scrambler
        }
    }
    
    cout << "TX: " << tx_symbols.size() << " total symbols" << endl;
    cout << "TX scrambler count: " << tx_scr.count() << endl;
    
    // RX - Option 1: Process all symbols, ignore probe content
    ...
    while (rx_data_count < BLOCK_SYMBOLS && rx_idx < (int)tx_symbols.size()) {
        // 32 data symbols
        for (int i = 0; i < UNKNOWN_LEN && rx_data_count < BLOCK_SYMBOLS && rx_idx < (int)tx_symbols.size(); i++) {
            int position = tx_symbols[rx_idx++];
        ...
        // 16 probe symbols - skip symbols and advance scrambler
        for (int i = 0; i < KNOWN_LEN && rx_idx < (int)tx_symbols.size(); i++) {
            rx_idx++;
            rx_scr.next();
        }
    }
    
    cout << "RX scrambler count: " << rx_scr.count() << endl;
