/**
 * Verify the encode/decode chain without frame structure
 */
#include <iostream>
#include <vector>
...
int main() {
    ...
    // TX: Message -> FEC -> Interleave -> Gray -> Scramble
    vector<int> msg_bits;
    ...
    for (size_t i = 0; i < BLOCK_SYMBOLS; i++) {
    ...
    cout << "TX: " << tx_symbols.size() << " symbols (no frame breaks)" << endl;
    
    // RX: Descramble -> Gray decode -> Deinterleave -> Viterbi
    RefDataScrambler rx_scr;
    RefDeinterleaver deinterleaver(ROWS, COLS, ROW_INC, COL_INC);
    
    for (size_t i = 0; i < BLOCK_SYMBOLS; i++) {
        int gray = (tx_symbols[i] - rx_scr.next() + 8) % 8;
