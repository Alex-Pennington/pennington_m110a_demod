/**
 * Decode M1200S with correct QPSK mapping
 */
#include <iostream>
#include <fstream>
#include <vector>
...
int main() {
    std::cout << "=== Decode M1200S with Correct QPSK ===" << std::endl;
    ...
    // 8-PSK constellation (only use 0,2,4,6 for QPSK)
    ...
    while (sym_idx < result.data_symbols.size()) {
        for (int d = 0; d < data_per_frame && sym_idx < result.data_symbols.size(); d++, sym_idx++) {
            ...
            // Find best QPSK point (0, 2, 4, 6)
            int best = 0;
            float best_corr = -1e9f;
            for (int i = 0; i < 8; i += 2) {
                float corr = desc.real() * con[i].real() + desc.imag() * con[i].imag();
                if (corr > best_corr) { best_corr = corr; best = i; }
            }
            
            float soft = std::min(best_corr * 40.0f, 127.0f);
            
            // MS-DMT QPSK soft bit mapping (Gray code)
            // +soft = bit 0, -soft = bit 1
            float sd0, sd1;
            switch (best) {
                case 0: sd0 = soft; sd1 = soft; break;   // 00
                case 2: sd0 = soft; sd1 = -soft; break;  // 01
                case 4: sd0 = -soft; sd1 = -soft; break; // 11
                case 6: sd0 = -soft; sd1 = soft; break;  // 10
                default: sd0 = sd1 = 0; break;
            }
            soft_bits.push_back(sd0);
            soft_bits.push_back(sd1);
        }
    ...
    std::cout << "Decoded bytes: " << bytes.size() << std::endl;
    std::cout << "ASCII: \"";
    for (uint8_t b : bytes) {
        char c = b;
