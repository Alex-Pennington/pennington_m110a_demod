/**
 * Decode whatever message is actually in the PCM file
 * Using the RECEIVED symbols (not expected)
 */
#include <iostream>
#include <fstream>
#include <vector>
...
int main() {
    ...
    cout << "Extracted " << result.data_symbols.size() << " symbols from PCM" << endl;
    ...
    cout << "\nDecoded message (LSB-first):" << endl;
    string output;
    ...
    // Also show hex
    cout << "\nFirst 60 bytes (hex):" << endl;
    for (size_t i = 0; i + 8 <= decoded.size() && i/8 < 60; i += 8) {
        uint8_t byte = 0;
        for (int j = 0; j < 8; j++) if (decoded[i + j]) byte |= (1 << j);
        printf("%02X ", byte);
        if ((i/8 + 1) % 20 == 0) cout << endl;
    }
    cout << endl;
    ...
    cout << "\nMatches with expected message: " << matches << "/54" << endl;
