/**
 * Decode with correct convolutional interleaver load/fetch pattern
 * 
 * Interleaver (TX):
 * - Load: row += row_inc, col += 1 if row wraps
 * - Fetch: row += 1, col += col_inc, special col_last handling
 *
 * Deinterleaver (RX):
 * - Load: row += 1, col += col_inc, special col_last handling  
 * - Fetch: row += row_inc, col += 1 if row wraps
 */
#include <iostream>
#include <fstream>
#include <vector>
...
public:
    ...
    // Load mirrors interleaver fetch pattern
    ...
    // Fetch mirrors interleaver load pattern
    int8_t fetch() {
...
int try_decode(const vector<complex<float>>& data_symbols, int start, bool show) {
    ...
    if (symbols_processed < BLOCK_SYMBOLS) return 0;
