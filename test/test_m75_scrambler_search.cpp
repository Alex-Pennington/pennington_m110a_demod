/**
 * Search for correct scrambler initialization
 */
...
std::string try_decode(const std::vector<complex_t>& symbols_4800, 
                        int offset, int scrambler_start) {
    ...
    if (soft_bits.size() < 90) return "(not enough)";
    ...
    for (size_t i = 0; i + 7 < decoded_bits.size(); i += 8) {
        ...
        if (byte >= 32 && byte < 127) result += (char)byte;
        else { char buf[8]; sprintf(buf, "[%02x]", byte); result += buf; }
    }
    return result;
}
...
int main() {
    ...
    std::cout << "=== Searching scrambler start positions ===\n\n";
    
    // Try all 160 scrambler positions at best offset
    int best_offset = 3838;
    std::cout << "At offset " << best_offset << ":\n";
    
    for (int scr = 0; scr < 160; scr += 8) {
        std::string out = try_decode(symbols_4800, best_offset, scr);
        if (out.find("Hello") != std::string::npos || 
            out.find("ello") != std::string::npos ||
            out.find("Hell") != std::string::npos) {
            std::cout << "*** FOUND at scr=" << scr << ": " << out << "\n";
        }
    }
    
    // Fine search around promising values
    std::cout << "\nFine search with all scrambler values:\n";
    for (int scr = 0; scr < 160; scr++) {
        std::string out = try_decode(symbols_4800, best_offset, scr);
        // Check for 'H' as first printable char
        if (!out.empty() && out[0] == 'H') {
            std::cout << "scr=" << scr << ": " << out << "\n";
        }
    }
    
    // Try offset 1572 too
    best_offset = 1572;
    std::cout << "\nAt offset " << best_offset << ":\n";
    for (int scr = 0; scr < 160; scr++) {
        std::string out = try_decode(symbols_4800, best_offset, scr);
        if (!out.empty() && out[0] == 'H') {
            std::cout << "scr=" << scr << ": " << out << "\n";
