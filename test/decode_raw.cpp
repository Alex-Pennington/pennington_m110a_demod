/**
 * Just decode the raw data - no assumptions about message
 */
#include <iostream>
#include <fstream>
#include <vector>
...
int main() {
    ...
    cout << "\n=== Approach 1: Standard decode ===" << endl;
    {
    ...
    // Approach 2: Skip probe but DON'T advance scrambler for probe
    cout << "\n=== Approach 2: Don't advance scrambler on probe ===" << endl;
    {
        RefDataScrambler scr;
        RefDeinterleaver deint(ROWS, COLS, ROW_INC, COL_INC);
        ...
        while (data_count < BLOCK_SYMBOLS && idx < (int)result.data_symbols.size()) {
            ...
            // Skip probe but DON'T advance scrambler
            for (int i = 0; i < 16 && idx < (int)result.data_symbols.size(); i++) {
                idx++;
                // scr.next();  // Don't advance!
            }
        }
    ...
    // Approach 3: Process ALL symbols continuously (no frame structure)
    cout << "\n=== Approach 3: All symbols, no frame structure ===" << endl;
    {
        RefDataScrambler scr;
        RefDeinterleaver deint(ROWS, COLS, ROW_INC, COL_INC);
        
        for (int i = 0; i < BLOCK_SYMBOLS && i < (int)result.data_symbols.size(); i++) {
