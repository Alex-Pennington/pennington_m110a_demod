...
public:
    ...
    bool set_afc_mode(const std::string& afc_type) {
        if (afc_type == "LEGACY") afc_mode_ = m110a::api::AFCMode::LEGACY;
        else if (afc_type == "MULTICHANNEL") afc_mode_ = m110a::api::AFCMode::MULTICHANNEL;
        else if (afc_type == "EXTENDED") afc_mode_ = m110a::api::AFCMode::EXTENDED;
        else if (afc_type == "MOOSE") afc_mode_ = m110a::api::AFCMode::MOOSE;
        else return false;
        afc_type_ = afc_type;
        return true;
    }
    ...
    bool run_test(const ModeInfo& mode, 
        ...
        cfg.afc_mode = afc_mode_;  // Apply AFC mode setting
    ...
    std::unique_ptr<ITestBackend> clone() const override {
        ...
        backend->afc_mode_ = afc_mode_;
        backend->afc_type_ = afc_type_;
...
private:
    ...
    m110a::api::AFCMode afc_mode_ = m110a::api::AFCMode::MOOSE;
    std::string afc_type_ = "MOOSE";
    ...
    /**
     * Apply frequency offset to passband signal using Hilbert transform
     * 
     * To shift a real signal s(t) by f Hz:
     *   s_shifted(t) = s(t)*cos(2*pi*f*t) - H{s(t)}*sin(2*pi*f*t)
    ...
    void apply_freq_offset(std::vector<float>& samples, float offset_hz, float sample_rate = 48000.0f) {
        ...
        // Compute Hilbert transform using FIR filter
        // h[n] = 2/(pi*n) for odd n, 0 for even n
        const int half_len = 64;  // Filter half-length (total = 129 taps)
        std::vector<float> hilbert(N, 0.0f);
        
        for (int i = 0; i < N; i++) {
            float sum = 0.0f;
            for (int k = -half_len; k <= half_len; k++) {
                if (k == 0) continue;  // h[0] = 0
                if (k % 2 == 0) continue;  // h[even] = 0
                int idx = i - k;
                if (idx >= 0 && idx < N) {
                    // h[k] = 2/(pi*k) with Hamming window
                    float h = 2.0f / (pi * k);
                    float window = 0.54f - 0.46f * std::cos(2.0f * pi * (k + half_len) / (2 * half_len));
                    sum += samples[idx] * h * window;
                }
            }
            hilbert[i] = sum;
        }
        
        // Apply frequency shift: s'(t) = s(t)*cos(wt) - H{s(t)}*sin(wt)
        float phase = 0.0f;
        float phase_inc = 2.0f * pi * offset_hz / sample_rate;
        for (int i = 0; i < N; i++) {
            float c = std::cos(phase);
            float s = std::sin(phase);
            samples[i] = samples[i] * c - hilbert[i] * s;
            phase += phase_inc;
