...
inline void generate_markdown_report(const std::string& filename, 
    ...
    report << "## Key\n\n";
    report << "### Modes\n";
    report << "- **S (Short)**: 0.6 second interleave - lower latency, less robust\n";
    report << "- **L (Long)**: 4.8 second interleave - higher latency, more robust\n";
    report << "- Data rates: 75, 150, 300, 600, 1200, 2400 BPS\n\n";
    
    report << "### Channel Conditions\n";
    report << "- **Clean**: No impairments (SNR=30dB, no offset, no multipath)\n";
    report << "- **AWGN_xxdB**: Additive White Gaussian Noise at specified SNR\n";
    report << "- **Freq_xxHz**: Frequency offset in Hz\n";
    report << "- **MP_xxms**: Multipath with specified delay spread\n";
    report << "- **Combo**: Combined impairments (realistic HF conditions)\n\n";
    
    report << "### Metrics\n";
    report << "- **Pass Rate**: Percentage of tests that decoded correctly\n";
    report << "- **BER**: Bit Error Rate (0.0 = perfect, higher = more errors)\n";
    report << "- **Rating**: A+ (100%), A (95%+), B (80%+), C (60%+), F (<60%)\n\n";
...
inline void write_progressive_csv_header(const std::string& filename, 
    ...
    csv << "#\n";
    csv << "# === KEY ===\n";
    csv << "# Mode: Data rate in BPS + S(Short interleave=0.6s) or L(Long interleave=4.8s)\n";
    csv << "#   75S/75L, 150S/150L, 300S/300L, 600S/600L, 1200S/1200L, 2400S/2400L\n";
    if (snr) {
        csv << "# Min_SNR_dB: Minimum SNR (dB) at which mode decodes correctly\n";
        csv << "#   Lower is better. Negative values = works below noise floor\n";
        csv << "#   Value of 30.0 = mode failed even at max SNR (broken)\n";
    }
    if (freq) {
        csv << "# Max_Freq_Offset_Hz: Maximum frequency offset tolerance (+/-)\n";
        csv << "#   Higher is better. Tests AFC (Automatic Frequency Control)\n";
        csv << "#   Value of 0.0 = mode failed even with no offset (broken)\n";
    }
    if (multipath) {
        csv << "# Max_Multipath_Samples: Maximum multipath delay tolerance (samples @ 48kHz)\n";
        csv << "# Max_Multipath_ms: Same value converted to milliseconds\n";
        csv << "#   Higher is better. Tests equalizer multipath handling\n";
        csv << "#   Value of 0 = mode failed even with clean channel (broken)\n";
    }
    csv << "#\n";
...
// ============================================================
// Progressive Test Report Generation
// ============================================================

inline void generate_progressive_markdown_report(
    const std::string& filename,
    const std::map<std::string, std::map<std::string, ProgressiveResult>>& all_eq_results,
    int duration_seconds,
    const std::string& backend_name,
    bool use_auto_detect,
    bool prog_snr, bool prog_freq, bool prog_multipath,
    const std::string& hwid = "") {
    
    std::ofstream report(filename);
    if (!report.is_open()) {
        std::cerr << "Cannot create progressive report: " << filename << std::endl;
        return;
    }
    ...
    report << "# M110A Modem Progressive Test Report\n\n";
    report << "## Test Information\n";
    report << "| Field | Value |\n";
    report << "|-------|-------|\n";
    report << "| **Version** | " << m110a::version() << " |\n";
    ...
    report << "| **Backend** | " << backend_name << " |\n";
    report << "| **Mode Detection** | " << (use_auto_detect ? "AUTO" : "KNOWN") << " |\n";
    report << "| **Test Date** | " << std::put_time(tm, "%B %d, %Y %H:%M") << " |\n";
    report << "| **Duration** | " << duration_seconds << " seconds |\n";
    report << "| **Tests** | ";
    if (prog_snr) report << "SNR ";
    if (prog_freq) report << "Freq ";
    if (prog_multipath) report << "Multipath ";
    report << "|\n\n";
    ...
    if (prog_snr) {
        report << "### Min SNR (dB)\n";
        report << "- Minimum Signal-to-Noise Ratio at which the mode decodes correctly\n";
        report << "- **Lower is better** (negative values = works below noise floor)\n";
        report << "- Value of **30.0 dB** = mode failed even at maximum SNR (broken)\n\n";
    }
    if (prog_freq) {
        report << "### Max Freq Offset (Hz)\n";
        report << "- Maximum frequency offset the AFC (Automatic Frequency Control) can correct\n";
        report << "- **Higher is better** (more tolerance to tuning errors)\n";
        report << "- Value of **0.0 Hz** = mode failed even with no offset (broken)\n\n";
    }
    if (prog_multipath) {
        report << "### Max Multipath (samples/ms)\n";
        report << "- Maximum multipath delay spread the equalizer can handle\n";
        report << "- **Higher is better** (more tolerance to HF channel distortion)\n";
        report << "- Samples @ 48kHz sample rate (1 sample = 0.0208 ms)\n";
        report << "- Value of **0** = mode failed even with clean channel (broken)\n\n";
    }
    
    report << "---\n\n";
    
    // Results for each equalizer
    for (const auto& [eq, progressive_results] : all_eq_results) {
        report << "## Equalizer: " << eq << "\n\n";
        
        // Build header
        report << "| Mode |";
        if (prog_snr) report << " Min SNR (dB) |";
        if (prog_freq) report << " Max Freq (Hz) |";
        if (prog_multipath) report << " Max Multipath |";
        report << "\n";
        ...
        for (const auto& [name, result] : progressive_results) {
            report << "| " << name << " |";
            if (prog_snr) {
                report << " " << std::fixed << std::setprecision(2) << result.snr_limit_db << " |";
            }
            if (prog_freq) {
                report << " +/-" << std::fixed << std::setprecision(2) << result.freq_offset_limit_hz << " |";
            }
            if (prog_multipath) {
                float ms = result.multipath_limit_samples / 48.0f;
                report << " " << result.multipath_limit_samples << " (" 
    ...
    report << "---\n\n";
    report << "*Report generated automatically by M110A Test Suite*\n";
