 ...
 * through the Brain Modem server interface. This validates the complete TX â†’ PCM â†’ RX
...
// Stats for mode Ã— channel combinations
...
float run_progressive_freq_test(ServerConnection& conn, const ModeInfo& mode,
    ...
    auto test_freq = [&](float freq) -> bool {
        ...
        std::cout << "\r  Freq Â±" << std::setw(5) << std::fixed << std::setprecision(1) 
...
ProgressiveResult run_progressive_tests(ServerConnection& conn, const ModeInfo& mode,
    ...
    if (test_freq) {
        ...
        std::cout << "  -> Limit: Â±" << result.freq_offset_limit_hz << " Hz\n";
    }
...
void generate_report(const std::string& filename, 
    ...
    report << "---\n\n";
    report << "*Generated by exhaustive_server_test via Brain Modem interface*\n";
...
int main(int argc, char* argv[]) {
    ...
    if (progressive_mode) {
        ...
        for (const auto& [name, result] : progressive_results) {
            ...
            if (prog_freq) {
                std::cout << std::setw(10) << "Â±" << result.freq_offset_limit_hz << " Hz | ";
            }
            if (prog_multipath) {
