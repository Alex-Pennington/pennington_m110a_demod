...
static classParam classStat600[TRACK_NUM_600];                 /* class of subframes */
static pitTrackParam600 pitTrack600[TRACK_NUM_600];               /* pitch of subframes */

Shortword   top_lpc[LPC_ORD];
Shortword   tmp_lpc[LPC_ORD];
Shortword   mem_lpc[NF600][LPC_ORD];

/* Prototype */

static void melp_ana (Shortword sp_in[], struct melp_param *par, Shortword subnum);
...
void analysis (Shortword sp_in[], struct melp_param *par)
{
	...
	Shortword local_rate;  /* Local copy of rate to avoid corruption issues */

	/* Save rate to local variable - the global can get corrupted during 
	 * 600 bps processing (memory overwrite bug in inner functions) */
	local_rate = rate;

	if (local_rate == RATE2400)
		num_frames = 1;

	if (local_rate == RATE1200)
		num_frames = NF;

	if (local_rate == RATE600)
		num_frames = NF600;
	...
	for (i = 0; i < num_frames; i++)
	{
		...
		if (local_rate == RATE2400)
			dc_rmv(&sp_in[i*FRAME], &hpspeech600[OLD_IN_BEG], dcdelin, \
											dcdelout_hi, dcdelout_lo, FRAME);
		if (local_rate == RATE1200)
			dc_rmv(&sp_in[i*FRAME], &hpspeech600[IN_BEG + i*FRAME], dcdelin, \
											dcdelout_hi, dcdelout_lo, FRAME);
		if (local_rate == RATE600)
			dc_rmv(&sp_in[i*FRAME], &hpspeech600[IN_BEG + i*FRAME], dcdelin, \
											dcdelout_hi, dcdelout_lo, FRAME);
		
		melp_ana (&hpspeech600[i*FRAME], &par[i], i);

		/* save lpc coefficients */
		if (local_rate ==  RATE600)
            v_equ(mem_lpc[i], tmp_lpc, LPC_ORD);
	}

	/* --- New routine to refine the parameters for block ------------------- */
	
	if ((local_rate == RATE1200) && (REFINEMENT1200 == TRUE))
		sc_ana (par);

	if ((local_rate == RATE600) && (REFINEMENT600 == TRUE))
		sc_ana600 (par);
	...
	if (local_rate == RATE600)
	{
		...
		VOI_voicing_quantization_s (par, &quant_par600.voicing_iq);

		/* --- Encoding mode determination ---------------------------------- */
		
		MODE_encoding_mode (&quant_par600);
	}
	...
	if (local_rate == RATE600)
	{
		MSVQ_lsf_quantization_s (par, &quant_par600);
	}
	...
	if (local_rate == RATE600)
	{
		QPIT_encoding_s (par, &quant_par600);
	}
	...
	if (local_rate == RATE600)
	{
		GAIN_gain_quantization_s (par, &quant_par600);
	}
	...
	if (local_rate == RATE600)
	{
		/* no jitter information is transmitted */
		for (i = 0; i < num_frames; i ++)
			par[i].jitter = 0;
	}
	...
	if (local_rate != RATE600)
	{
		for (i = 0; i < num_frames; i++)
		{
			...
			if (!par[i].uv_flag)
			{
				...
				zerflt (&hpspeech600[(i*FRAME + FRAME_END - (LPC_FRAME/2))], \
	...
	if (local_rate == RATE600)
	{
		/* no fourier coefficient is transmitted */
	}
	...
	if (local_rate == RATE600)
		WRS_build_stream (&quant_par600, chbuf600);

	/* --- Update delay buffers for next block ------------------------------ */
	
	v_equ (hpspeech600, &(hpspeech600[num_frames*FRAME]), IN_BEG);
															...
															Shortword subnum)
{
	...
	/* jitter is set to 0 for 600 bps mode ... */
	if (rate == RATE600)
		par->jitter = 0;
	...
	if (rate == RATE600){
        ...
        cur_track = (Shortword) (CUR_TRACK + subnum * PIT_SUBNUM);
        for (i = 0; i < PIT_SUBNUM; i++){
            pitchAuto600(&speech[FRAME_END + i*PIT_SUBFRAME + PIT_COR_LEN/2],
                      &pitTrack600[cur_track + i + 1],
                      &classStat600[cur_track + i + 1]);

            classify(&speech[FRAME_END + i * PIT_SUBFRAME + PIT_SUBFRAME/2],
                     &classStat600[cur_track + i + 1], auto_corr);
        }
    }
...
void melp_ana_init()
{
    ...
    v_zap (hpspeech600, IN_BEG + BLOCK600);   /* speech[] is declared IN_BEG + BLOCK */
    ...
    for (i = 0; i < TRACK_NUM_600; i++){
        fill(pitTrack600[i].pit, FIFTY_Q0, NODE600); /* !!! (12/10/99) */
        fill(pitTrack600[i].weight, ONE_Q15, NODE600);

        classStat600[i].classy = UNVOICED;
        classStat600[i].subEnergy = X28_Q11;
        classStat600[i].zeroCrosRate = X05_Q15;
        classStat600[i].peakiness = ONE_Q11;
        classStat600[i].corx = X02_Q15;
    }
...
** Function:        sc_ana600
**
** Description:     The analysis routine for block (600bps only)
**
** Arguments:
**
**  melp_param  *par    ---- output encoded melp parameters
...
/*    Shortword   index; */
    ...
    for (i = 0; i < NF600; i++){
        ...
        /* update silence energy ... */
        if ((classStat600[curTrack].classy == SILENCE) &&
            (classStat600[curTrack - 1].classy == SILENCE)){
            /*  silenceEn = log10(EN_UP_RATE * pow(10.0, silenceEn) +
                                  (1.0 - EN_UP_RATE) *
                                  pow(10.0, classStat600[curTrack].subEnergy)); */
            silenceEn = updateEn(silenceEn, EN_UP_RATE_Q15,
                                 classStat600[curTrack].subEnergy);
        }
    }
	...
	if (!uv[1] && !uv[2] && !uv[3] && \
							!subenergyRelation1(classStat600, curTrack))
	{
		/* process only voiced frames, check if it is offset first ... */
		if ((voicedCnt < 2) || subenergyRelation2 (classStat600, curTrack))
		{
			/* Got onset position, should look forward for pitch ... */
			pitCand = pitLookahead600(&pitTrack600[curTrack], 3);
		...
		else if (!uv[0])
		{
				...
				else
				{
					...
					par[0].pitch = add(temp1, temp2);                   /* Q7 */
							...
							((ratio(par[1].pitch, prev_pitch) < X015_Q15) || \
								(ratio(par[2].pitch, prev_pitch) < X015_Q15)))
			{
				index1 = trackPitch600(prev_pitch, &pitTrack600[curTrack]);
				
				pitCand = shl(pitTrack600[curTrack].pit[index1], 7); /* !!! (12/10/99) */
				
				index2 = trackPitch600(par[0].pitch, &pitTrack600[curTrack]);
				
				w1_w2 = sub(pitTrack600[curTrack].weight[index1], \
										pitTrack600[curTrack].weight[index2]);
			...
			else if ((L_ratio(par[0].pitch,(Longword)(prev_pitch*2)) < X008_Q15) || \
				(L_ratio(par[0].pitch,(Longword)(prev_pitch*3)) < X008_Q15))
			{
				/* possible it is a double pitch ... */
				pitCand = pitLookahead600(&pitTrack600[curTrack], 4);
	...
	/* The modified second frame (same processing as first frame) ... */
							...
							!subenergyRelation1(classStat600, curTrack))
	{
		...
		if ((voicedCnt < 2) || subenergyRelation2 (classStat600, curTrack))
		{
			...
			if (ratio (par[1].pitch, pitCand) > X015_Q15)
			{
				if (ratio (pitCand, par[2].pitch) < X015_Q15)
					par[1].pitch = pitCand;
				else if ((ratio (par[2].pitch, par[3].pitch) < X015_Q15) &&\
							(ratio(par[1].pitch, par[2].pitch) > X015_Q15))
					par[1].pitch = par[2].pitch;
				else if (ratio (par[1].pitch, par[2].pitch) > X015_Q15)
					par[1].pitch = pitCand;
			}
		}
		else if (!uv[1])
		{
														...
														(index1 * index2 < 0))
			{
				...
				else
				{
					...
					par[1].pitch = add(temp1, temp2);                   /* Q7 */
							...
							((ratio(par[2].pitch, prev_pitch) < X015_Q15) || \
								(ratio(par[3].pitch, prev_pitch) < X015_Q15)))
			{
				index1 = trackPitch600(prev_pitch, &pitTrack600[curTrack]);
				
				pitCand = shl(pitTrack600[curTrack].pit[index1], 7); /* !!! (12/10/99) */
				
				index2 = trackPitch600(par[1].pitch, &pitTrack600[curTrack]);
				...
				if (multiCheck(par[1].pitch, pitCand) < X008_Q15)
				{
					if (((par[1].pitch > pitCand) && (w1_w2 > MX02_Q15)) || \
															(w1_w2 > X02_Q15))
						par[1].pitch = pitCand;
				}
				else if (w1_w2 > MX01_Q15)
			...
			else if ((L_ratio(par[1].pitch,(Longword)(prev_pitch*2)) < X008_Q15) || \
				(L_ratio(par[1].pitch,(Longword)(prev_pitch*3)) < X008_Q15))
			{
				/* possible it is a double pitch ... */
				pitCand = pitLookahead600(&pitTrack600[curTrack], 4);
	...
	/* The modified third frame (same processing as second frame) ... */
	
	prev_pitch = shl(shr(par[1].pitch, 7), 7);     /* Rounding to Q7 integer. */
	curTrack = CUR_TRACK + 4;
	if (!uv[3])
		voicedCnt++;
	else
	...
	if (!uv[3] && !subenergyRelation1(classStat600, curTrack))
	{
		if ((voicedCnt < 2) || subenergyRelation2(classStat600, curTrack))
		{
			/* Got onset position, should look forward for pitch ... */
			...
			if (ratio(par[2].pitch, pitCand) > X015_Q15)
			{
				if (ratio(pitCand, par[3].pitch) < X015_Q15)
					par[2].pitch = pitCand;
				else
				{
					npitch = pitLookahead600(&pitTrack600[curTrack + 1], 3);
					
					if (ratio(npitch, par[3].pitch) < X015_Q15)
					{
						if (ratio(pitCand, npitch) < X015_Q15)
							par[2].pitch = pitCand;
						else if (ratio(par[2].pitch, npitch) > X015_Q15)
		...
		else if (!uv[2])
		{
			...
			temp1 = sub(par[2].pitch, prev_pitch);
			index1 = shr(temp1, 7);
			temp2 = sub(par[3].pitch, par[2].pitch);
			index2 = shr(temp2, 7);
			pitCand = pitLookahead600(&pitTrack600[curTrack], 3);
														...
														(index1 * index2 < 0))
			{
				/* here is a pitch jump ... */
				if ((ratio(prev_pitch, pitCand) < X015_Q15) || \
									(ratio(pitCand, par[3].pitch) < X02_Q15))
					par[2].pitch = pitCand;
				else
				{
					/* par[2].pitch = (prev_pitch + par[3].pitch)/2; */
					temp1 = shr(prev_pitch, 1);
					temp2 = shr(par[3].pitch, 1);
					par[2].pitch = add(temp1, temp2);                   /* Q7 */
				}
			}
			else if ((ratio(par[2].pitch, prev_pitch) > X015_Q15) && \
							((ratio(par[3].pitch, prev_pitch) < X015_Q15) || \
									(ratio(pitCand, prev_pitch) < X015_Q15)))
			{
				if (ratio(pitCand, prev_pitch) < X015_Q15)
				...
				else
				{
					...
					index2 = trackPitch600(par[2].pitch, &pitTrack600[curTrack]);
					...
					if (multiCheck(par[2].pitch, pitCand) < X008_Q15)
					{
						if (((par[2].pitch > pitCand) && (w1_w2 > MX02_Q15)) || \
															(w1_w2 > X02_Q15))
							par[2].pitch = pitCand;
					}
					else if (w1_w2 > MX01_Q15)
			...
			else if ((L_ratio(par[2].pitch,(Longword)(prev_pitch*2)) < X008_Q15) || \
					(L_ratio(par[2].pitch,(Longword)(prev_pitch*3)) < X008_Q15))
			{
				/* possible it is a double pitch ... */
				pitCand = pitLookahead600(&pitTrack600[curTrack], 4);
	...
	/* The modified fourth frame (same as third frame) ... */
	prev_pitch = shl(shr(par[2].pitch, 7), 7);
	curTrack = CUR_TRACK + 6;
	if (!uv[4])
		voicedCnt++;
	else
		voicedCnt = 0;
	
	if (!uv[4] && (classStat600[curTrack + 1].classy == VOICED) && \
						(classStat600[curTrack + 2].classy == VOICED) && \
								!subenergyRelation1(classStat600, curTrack))
	{
		if ((voicedCnt < 2) || subenergyRelation2(classStat600, curTrack))
		{
			/* Got onset position, should look forward for pitch ... */
			pitCand = pitLookahead600(&pitTrack600[curTrack], 2);
			
			if (ratio(par[3].pitch, pitCand) > X015_Q15)
			{
				npitch = pitLookahead600(&pitTrack600[curTrack + 1], 1);
				
				if (ratio(npitch, pitCand) < X015_Q15)
					par[3].pitch = pitCand;
				else if (ratio(par[3].pitch, npitch) >= X015_Q15)
					par[3].pitch = npitch;
			}
		}
		else if (!uv[3])
		{
			...
			temp1 = sub(par[3].pitch, prev_pitch);
			index1 = shr(temp1, 7);

			temp2 = sub(pitCand, par[3].pitch);
			index2 = shr(temp2, 7);
														...
														(index1 * index2 < 0))
			{
				...
				{
					index1 = trackPitch600(pitCand, &pitTrack600[curTrack]);
					index2 = trackPitch600(par[3].pitch, &pitTrack600[curTrack]);
					
					w1_w2 = sub(pitTrack600[curTrack].weight[index1], \
										pitTrack600[curTrack].weight[index2]);
					
					if (multiCheck(par[3].pitch, pitCand) < X008_Q15)
					{
						if (((par[3].pitch > pitCand) && (w1_w2 > MX02_Q15)) || \
															(w1_w2 > X02_Q15))
							par[3].pitch = pitCand;
					}
					else
					{
						...
						if (multiCheck(par[3].pitch, pitCand) < X008_Q15)
						{
							if (((par[3].pitch > pitCand) && \
									(w1_w2 > MX02_Q15)) || (w1_w2 > X02_Q15))
								par[3].pitch = pitCand;
						}
						else
						{
							/*par[3].pitch = (prev_pitch + pitCand)/2; */
							temp1 = shr(prev_pitch, 1);
							temp2 = shr(pitCand, 1);
							par[3].pitch = add(temp1, temp2);           /* Q7 */
						}
					}
				}
			}
    ...
    for (i = 0; i < NF600; i++)
	{
        ...
        if (voicedEn < classStat600[curTrack].subEnergy)
            voicedEn = classStat600[curTrack].subEnergy;
    }

    /* Update class and pitch structures ... */
    for (i = 0; i < TRACK_NUM_600 - NF600*PIT_SUBNUM; i++){
        classStat600[i] = classStat600[i + NF600 * PIT_SUBNUM];
        pitTrack600[i] = pitTrack600[i + NF600 * PIT_SUBNUM];
    }

    prev_uv = par[NF600 - 1].uv_flag;

    /* Set prev_pitch to par[NF - 1].pitch so that it is an integer for its   */
    /* Q value.  We first use shr() to truncate par[NF - 1].pitch and then    */
    /* use shl() to correct the Q value.                                      */

    prev_pitch = shl(shr(par[NF600 - 1].pitch, 7), 7);
