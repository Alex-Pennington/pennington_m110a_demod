...
static void corPeak600(Shortword inbuf[], pitTrackParam600 *pitTrack,
                                                    classParam *classStat);
               ...
               classParam *classStat)
{
    ...
    /* ------ Calculate the autocorrelation function ------- */
    corPeak600(ivbuf, pitTrack, classStat);
}
...
static void lpfilt(Shortword inbuf[], Shortword lpbuf[], Shortword len)
{
    ...
    for (i = 0; i < len; i++){
        ...
        lpbuf[PIT_COR_LEN - len + i] = melp_round(L_sum);                    /* Q0 */
    }
}
...
static void ivfilt(Shortword ivbuf[], Shortword lpbuf[], Shortword len)
{
    ...
    r_coeff[0] = melp_round(L_temp);                     /* normalized r0 */
    for (i = 1; i < 3; i++){
        L40_sum = 0;
        for (j = i; j < PIT_COR_LEN; j++)
            L40_sum = L40_mac(L40_sum, lpbuf[j], lpbuf[j-i]);
        L_temp = (Longword) L40_shl(L40_sum, shift);
        r_coeff[i] = melp_round(L_temp);
    }
    ...
    for (i = 0; i < len; i++){
        ...
        ivbuf[PIT_COR_LEN-len+i] = melp_round(L_shl(L_temp, 3));
    }
}
...
/*==============================================================*
 *          corPeak600: fill pitTrack structure                    *
 *  inbuf       ---- input data buffer (Q12)                    *
 *  pitTrack    ---- pitch pitTrackParam structure              *
 *  classStat   ---- classification paramters                   *
 *==============================================================*/
                                            ...
                                            classParam *classStat)
{
    ...
    Longword    L_r0, L_rk;
    Shortword   r0_shift, rk_shift, root;
    Shortword   gp[MAXPITCH + 1], peak[MAXPITCH + 1], corx[NODE600];      /* Q15 */
    ...
    /* ---- Modify time domain correlation by muliple checking ---- */
    for (i = 0; i < NODE600 - 1; i++){
        for (j = (Shortword) (i + 1); j < NODE600; j++){
            temp1 = pitTrack->pit[j];
            temp2 = pitTrack->pit[i];
            temp = temp2;
            while (temp < temp1){
...
Shortword pitLookahead600(pitTrackParam600 *pitTrack, Shortword num)
{
    ...
    for(i = sub(num, 1); i >= 0; i--){
        for(j = 0; j < NODE600; j++){
            index = trackPitch600(shl(pitTrack[i].pit[j], 7), &pitTrack[i + 1]);
